...
from core.xml_procesor import XMLProcessor
...
class DocumentosXMLApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Generador de Documentos DOCX desde XML")
        ...
        try:
            ...
            try:
                ...
                xml_data = self.xml_processor.read_xml(
                    ...
                    fecha_documento,
                    partida_numero,
                    "",  # Numero de oficio (ya no se usa)
                    ""   # Fecha de remisión (ya no se usa)
                )
                ...
                # Generar documentos
                self.update_status("Generando documentos DOCX...")
                
                # Generar los documentos necesarios - solo DOCX, sin PDF
                self.generate_docx_documents(
                    xml_data,
                    output_dir,
                    monto_formateado
                )
                ...
                self.update_status(f"✅ Documentos DOCX generados correctamente para {os.path.basename(xml_path)}.")
                messagebox.showinfo("Éxito", "Documentos DOCX generados correctamente")
    ...
    def generate_docx_documents(self, xml_data, output_dir, monto_formateado):
        """
        Genera solo documentos DOCX a partir de los datos XML.
        Reemplaza la llamada a DocumentGenerator.generate_all_documents
        para evitar la generación de PDF.
        
        Args:
            xml_data (dict): Datos extraídos del XML y de la interfaz
            output_dir (str): Directorio donde guardar los documentos
            monto_formateado (str): Monto formateado como string
        """
        try:
            # Obtener las rutas de las plantillas
            path_general = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "templates")
            
            # Generar documento de legalización de factura
            self.update_status("Generando legalización de factura (DOCX)...")
            path_legalizacion_factura = os.path.join(path_general, "legalizacion_factura.docx")
           
            docx_path = legalizacionFactura(path_legalizacion_factura, output_dir, xml_data)
            self.update_status(f"  ✓ Generado: {os.path.basename(docx_path)}")
            
            # Generar documento XML
            self.update_status("Generando documento XML (DOCX)...")
            path_crear_xml = os.path.join(path_general, "xml.docx")
            from generators.crearDocXml import crearXML
            docx_path = crearXML(path_crear_xml, output_dir, xml_data)
            self.update_status(f"  ✓ Generado: {os.path.basename(docx_path)}")
            
            # Generar legalización de XML
            self.update_status("Generando legalización de XML (DOCX)...")
            path_legalizacion_xml = os.path.join(path_general, "legalizacion_xml.docx")
            from generators.legalizacionXml import legalizacionXml
            docx_path = legalizacionXml(path_legalizacion_xml, output_dir, xml_data)
            self.update_status(f"  ✓ Generado: {os.path.basename(docx_path)}")
        
            
        except Exception as e:
            self.update_status(f"Error al generar documentos DOCX: {str(e)}")
            raise Exception(f"Error al generar documentos DOCX: {str(e)}")
